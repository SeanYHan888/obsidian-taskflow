...
**Bulk triage.** Choose *Select tasks* on the To-do or Projects header, or *Select multiple* on any row's menu to start with that task in hand — checkboxes appear across the working list and the backlogs. A bar at the panel's foot shows the count with *move to project* and bulk scheduling. `Esc` exits.
